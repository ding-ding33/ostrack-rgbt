...
def get_optimizer_scheduler(net, cfg):
    modality_repair_enabled = getattr(cfg.MODEL.BACKBONE, "ENABLE_MODALITY_REPAIR", False)
    local_finetune = getattr(cfg.TRAIN, "LOCAL_FINETUNE", False) and modality_repair_enabled
    ...
    if local_finetune:
        print("Local fine-tuning enabled. Only training these modules:")
        for n, p in net.named_parameters():
            p.requires_grad = any(key in n for key in local_finetune_keys)
            if p.requires_grad:
                print(n)
