...
class BATActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        if len(template_list) == 2:
            # torch.Size([2, 16, 4]) --> data['template_anno']
            template_bboxes = torch.tensor(data['template_anno'][0], dtype=torch.float32, device='cuda')  # 假设模板目标的bbox
            template_bboxes_masks = self.get_bboxes_masks(template_bboxes, B, H_T, W_T)
            self.template_bboxes_masks = template_bboxes_masks.view(B, -1)     
            self.template_masks_ = self.template_masks2 = torch.where(self.template_bboxes_masks, torch.tensor(1).to('cuda'), torch.tensor(0).to('cuda')).to('cuda')
            template_mask_list = []
            for i in range(B):
                template_mask_list.append(torch.cat([self.template_masks2[i].unsqueeze(0),self.template_masks[i].unsqueeze(0)],dim=0))
            self.template_masks2 = torch.cat(template_mask_list,dim=0)
            search_mask_list = []
            for i in range(B):
            ...
            out_dict = self.net(template=template_list,
                                ...
                                seq_name =None,
                                template_masks = self.template_masks2,
                                )
        else:
            # 计算FLOPs和参数量
            flops, params = profile(self.net, inputs=(template_list,
                                search_img,
                                box_mask_z,
                                ce_keep_rate,
                                False,
    ...
    def compute_losses(self, pred_dict, gt_dict,return_status=True):
        ...
        # load_loss = pred_dict["load_loss"]* 0.1
