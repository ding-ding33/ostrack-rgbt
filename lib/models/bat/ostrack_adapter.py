    ...
    def forward(self, template: torch.Tensor,
        ...
        if frame_id == 1:
            x, aux_dict,track_token = self.backbone(z=template, x=search,
                                        ...
                                        dynamic_template=None,Test=Test,template_masks=template_masks,frame_id=frame_id)
            self.dynamic_template = track_token #动态模板初始化
            ...
            x, aux_dict,track_token = self.backbone(z=template, x=search,
                                        ...
                                        ce_keep_rate=ce_keep_rate,
                                        return_last_attn=return_last_attn,
                                        dynamic_template = self.dynamic_template,Test=Test,template_masks=template_masks,frame_id=frame_id)
            # self.last_template = template
            self.dynamic_template = track_token #动态模板更新
